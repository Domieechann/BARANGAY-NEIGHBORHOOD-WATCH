...
<script>
        ...
        el.innerHTML = requests.map(r => `
          <div class="report-item" id="reset-row-${r.id}">
            ...
            <div style="display:flex;gap:6px;flex-shrink:0;">
              <button class="btn-approve" onclick="handleReset(${r.id}, 'approve', this)">✅ Approve</button>
              <button class="btn-reject"  onclick="handleReset(${r.id}, 'reject',  this)">❌ Reject</button>
            </div>
          </div>`).join('');
      })
      .catch(() => {
  ...
  function handleReset(id, action, btn) {
    ...
    .then(data => {
      if (data.success) {
        ...
        showToast(action === 'approve' ? '✅ Password na-reset na!' : '❌ Request rejected.');
      } else {
